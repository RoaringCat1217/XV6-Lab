...
int
kvmcopy(pagetable_t new, pagetable_t old)
{
...
kvminit()
{
  if (kernel_pagetable)
  {
    // copy kernel pagetable for user
    pagetable_t kpagetable = (pagetable_t)kalloc();
    if (kvmcopy(kpagetable, kernel_pagetable) < 0)
      return 0;
    // CLINT is lower than PLIC, unmap it
...
int
kvmmap_usr(pagetable_t kpagetable, uint64 va, uint64 pa, uint64 sz, int perm)
{
