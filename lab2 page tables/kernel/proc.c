...
growproc(int n)
{
  ...
  uint64 va;
  pte_t *pte1, *pte2;
  for (va = 0; va < PGROUNDUP(p->sz); va += PGSIZE)
  {
    pte1 = walk(p->pagetable, va, 0);
    pte2 = walk(p->kpagetable, va, 0);
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  uint64 va;
  pte_t *pte1, *pte2;
  for (va = 0; va < PGROUNDUP(np->sz); va += PGSIZE)
  {
    pte1 = walk(np->pagetable, va, 0);
    pte2 = walk(np->kpagetable, va, 0);
    if (PTE2PA(*pte1) != PTE2PA(*pte2))
    {
      printf("???\n");
    }
